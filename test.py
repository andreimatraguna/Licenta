...
def play(om):
    train_episodes=1
    train_length=250000000
    pozitie_torace=[]
    for episodes in range(1,train_episodes+1):
        om.reset_model()
        score=0
        for _step in range(1,train_length+1):
            om.render()
            n_state,reward,done,info=om.step(om.action_space.sample())
            score+=reward
            pozitie_torace.append(info['sol_distance'])
            #print(info)
            if episodes==train_episodes/2:
                if min(pozitie_torace)<6:
                    episodes-=1
                    break
            if done == True:
                break
        #print(str(score)+"episode"+str(episodes)+"\n\n\n")
def build_model(states,actions):
    model=Sequential()
    model.add(Flatten(input_shape=(1,states)))
    model.add(Dense(32,activation=activations.elu))
    model.add(Dense(32,activation=activations.elu))
    model.add(Dense(32,activation=activations.elu))
    model.add(Dense(32,activation=activations.elu))
    model.add(Dense(32,activation=activations.elu))
...
def build_agent(model,actions):
    policy =EpsGreedyQPolicy() #LinearAnnealedPolicy(inner_policy=EpsGreedyQPolicy(),value_max=0.4,value_min=-0.4,value_test=0.1,nb_steps=50,attr='eps')
    memory = SequentialMemory(limit=50000,window_length=1)
    dqn = DQNAgent(model=model,memory=memory,policy=policy,nb_actions=actions,nb_steps_warmup=5,target_model_update=1e-4)
    return dqn
om=HumanoidStandupEnv()
states=om.observation_space.shape[0]
actions=spaces.Discret(17).n
print(actions)
model=build_model(states,actions)
dqn = build_agent(model,actions)
dqn.compile(Adam(lr=1e-5),metrics=['mae'])#mae=mean absolte era
dqn.fit(om,nb_steps=160000,visualize=True,verbose=1)
...
class PPOAgent:
    ...
    def __init__(self, env_name, model_name=""):
        ...
        self.EPISODES = 2000 # total episodes to train through all environments
    ...
    def act(self, state):
        ...
        low, high = -0.4, 0.4 # -1 and 1 are boundaries of tanh
    ...
    def get_gaes(self, rewards, dones, values, next_values, gamma = 0.99, lamda = 0.74, normalize=True):
...
#                self.env.render()
                ...
                score += reward
                #print("episode"+str(self.episode)+"score"+str(score))
                if done:
                    self.episode += 1
                    average, SAVING = self.PlotModel(score, self.episode)
                    print("episode: {self.episode}/{self.EPISODES}, score: {score}, average: {np.mean(average):.2f} {SAVING}")
        ...
        while self.episode < self.EPISODES:
            ...
            for worker_id, parent_conn in enumerate(parent_conns):
                ...
                if done:
                    ...
                    print("episode: {self.episode}/{self.EPISODES}, worker: {worker_id}, score: {score[worker_id]}, average: {np.mean(average):.2f} {SAVING}")
        ...
        for e in range(101):
            ...
            while not done:
                ...
                if done:
                    ...
                    print("episode: {e}/{test_episodes}, score: {score}, average{average}")
                    break
        self.env.close()
